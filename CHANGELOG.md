...
- 新增独立 `model-requester` 公共模块，定义统一 `ModelMessage`、请求、响应和错误协议；
  MiniMax 已实现网络请求、厂商格式转换、工具调用、多模态输入和推理续传，其他厂商保留
  显式未实现入口。旧 MiniMax Provider 通过该公共接口兼容现有 TCB 调度链路。
- 新增独立 `agent-control-block` 数据协议，定义长期 Agent 身份、角色、名称、四态调度
  状态、文件权限、Skills、模型配置引用和 `AgentContext` 占位类型；暂不接入现有 TCB。

### 桌面启动恢复修复

- 修复取消/失败任务时未清除工作图活动节点引用，导致下次启动恢复校验失败、窗口无法打开。
  对旧版终态快照进行限定范围的兼容处理，保留任务结果和事件历史；新增 4 个回归用例。
- 启动初始化失败时显示具体错误并退出，避免只留下没有窗口的 Electron 进程。
- 将 Electron 沙箱预加载桥接脚本改为 CommonJS，修复 ESM 预加载失败导致界面拿不到
  `window.osAgent` 的问题；加载完成后显式显示并聚焦窗口。

### MiniMax M3 与多模态

- 对照 MiniMax 官方协议保留原生 assistant 消息、`reasoning_details` 和工具调用 ID，
  恢复匹配的 tool 消息；拒绝混合控制/业务调用及被截断的工具参数。
- 新增图片/视频附件、`media.read`、`image.generate` 和 `video.generate`；生成结果
  写入 Artifact 并在对话显示，图片工件可以再次通过 `artifact.read` 交给模型理解。
- MiniMax M3 原生映射 `video_url`，MOV Base64 使用 `video/mov`；Gemini 使用
  `inlineData`，不支持视频的适配器提前报错。修复大附件 Base64 校验栈溢出。
- 新增 SQLite 工具操作账本，恢复视频远端任务，防止未知提交结果被自动重复计费；
  生成工件支持跨事务恢复。文件写入/补丁记录前后摘要并原子替换，防止恢复时重复应用。
- 修复规范化路径绕过工作区根删除保护、递归删除未锁住子树、鉴权/额度错误反复重试、
  异步工具媒体丢失、上下文下拉框错误禁用和附件按钮 CSS 冲突。
- 沙箱探测同时验证允许写入和禁止写入，避免把后端启动失败当成隔离成功。
- 真实 MiniMax M3 文本工具闭环、图片输入/生成、生成图片识别和视频输入通过。
  视频生成返回 `2056` 套餐额度错误，未宣称真实成片成功。
- 当前自动测试 238 通过；4 个 macOS 进程隔离测试因当前环境无法启用后端而跳过。
  内核/桌面类型检查和生产构建通过；组件预览验证了图片显示、视频播放和附件菜单。
