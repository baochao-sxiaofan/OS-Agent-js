...
4. 支持优先级的单进程 Ready Queue。
5. 感知 RPM、TPM、并发额度和任务预算的请求准入控制。
6. Capability 驱动的工具权限校验。
7. 只读工具并行、有副作用工具串行的执行策略。
8. `READY -> RUNNING -> BLOCKED -> READY -> TERMINATED` 演示。
9. 状态机、调度、配额、预算和工具权限测试。

后续优先级：

1. 为处于 `READY` 的限流任务增加基于 `retryAt` 的自动唤醒。
2. 增加持久化数据库 Adapter 和进程崩溃恢复测试。
3. 增加人工审批、子 Agent 和资源锁对应的阻塞/唤醒协议。
4. 增加真实模型 Provider Adapter。
